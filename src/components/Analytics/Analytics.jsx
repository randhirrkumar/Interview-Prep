...
export default function Analytics() {
  ...
  const readinessLevel = stats.overallPct >= 80 ? { label: 'Interview Ready', color: 'text-green-400' }
    : stats.overallPct >= 50 ? { label: 'Good Progress', color: 'text-yellow-400' }
    : stats.overallPct >= 25 ? { label: 'Building Up', color: 'text-orange-400' }
    : { label: 'Just Started', color: 'text-red-400' }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="card">
        ...
        <p className="text-sm text-gray-400">Your study progress across all topics at a glance.</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { icon: CheckCircle, label: 'Completed', value: stats.totalDone, sub: `/ ${stats.totalQuestions}`, color: 'text-green-400' },
          { icon: TrendingUp, label: 'Overall', value: stats.overallPct + '%', sub: readinessLevel.label, color: readinessLevel.color },
          { icon: Calendar, label: 'Day Streak', value: streak, sub: 'days', color: 'text-orange-400' },
          { icon: Award, label: 'Days Studying', value: stats.daysStudying, sub: 'total', color: 'text-purple-400' },
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div className="font-semibold text-white">Overall Readiness</div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold text-white">Strong Topics</h2>
            <span className="text-xs text-gray-500">({stats.strong.length})</span>
          </div>
          {stats.strong.length === 0 ? (
            <div className="text-sm text-gray-600 text-center py-4">Complete 70%+ of any topic to see it here.</div>
          ) : (
              ...
              {stats.strong.map(t => (
                <div key={t.id}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-green-400">{t.done}/{t.total}</span>
                  </div>
        ...
        <div className="card">
          ...
          {stats.weak.length === 0 ? (
            <div className="text-sm text-gray-600 text-center py-4">No weak topics yet. Keep studying!</div>
          ) : (
            <div className="space-y-3">
              {stats.weak.map(t => (
                <div key={t.id}>
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold text-white">All Topics Breakdown</h2>
        </div>
        <div className="space-y-3">
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              <span className="text-gray-400">DSA Problems</span>
