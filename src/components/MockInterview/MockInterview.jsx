...
import { Play, RotateCcw, Clock, CheckCircle, XCircle, MinusCircle } from 'lucide-react'
    ...
    <div className="max-w-3xl mx-auto space-y-4 animate-fade-in">
      ...
      <div className="card border-l-4 border-blue-600">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 px-2 py-0.5 rounded">{q.topic}</span>
      ...
      {/* Think pad */}
      {!showAnswer && (
        <div className="card border-dashed border-gray-300 dark:border-gray-700">
          <div className="text-xs text-gray-400 mb-2">Your thinking space (formulate your answer mentally)</div>
          <textarea
            className="w-full bg-transparent text-sm text-gray-600 dark:text-gray-400 outline-none resize-none min-h-[80px] placeholder-gray-400 dark:placeholder-gray-700"
      ...
      {showAnswer && (
        <div className="space-y-4 animate-fade-in">
          <div className="card bg-green-50 border-green-300 dark:bg-green-950/20 dark:border-green-800/50">
            <div className="text-xs text-green-600 dark:text-green-400 font-semibold mb-2">MODEL ANSWER</div>
          ...
          {/* Self-assessment */}
          <div className="card">
            <div className="text-sm font-semibold text-gray-900 dark:text-white mb-3">How did you do?</div>
            <div className="flex gap-3">
              <button onClick={() => score('easy')} className="flex-1 flex flex-col items-center gap-1.5 p-3 bg-green-50 border border-green-300 dark:bg-green-900/20 dark:border-green-700 rounded-xl hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors">
                <CheckCircle className="text-green-500 dark:text-green-400" size={22} />
                <span className="text-xs text-green-600 dark:text-green-300 font-medium">Nailed It</span>
              </button>
              <button onClick={() => score('medium')} className="flex-1 flex flex-col items-center gap-1.5 p-3 bg-yellow-50 border border-yellow-300 dark:bg-yellow-900/20 dark:border-yellow-700 rounded-xl hover:bg-yellow-100 dark:hover:bg-yellow-900/40 transition-colors">
                <MinusCircle className="text-yellow-500 dark:text-yellow-400" size={22} />
                <span className="text-xs text-yellow-600 dark:text-yellow-300 font-medium">Partially</span>
              </button>
              <button onClick={() => score('hard')} className="flex-1 flex flex-col items-center gap-1.5 p-3 bg-red-50 border border-red-300 dark:bg-red-900/20 dark:border-red-700 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                <XCircle className="text-red-500 dark:text-red-400" size={22} />
                <span className="text-xs text-red-600 dark:text-red-300 font-medium">Missed It</span>
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5 animate-fade-in">
      <div className="card">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Simulate a real interview. Timer runs for each question. Self-assess your answers.</p>
      </div>
      ...
      <div className="card space-y-5">
        ...
        <div>
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">Number of Questions: {questionCount}</label>
        ...
        <div className="flex items-center gap-3 p-3 bg-gray-100 dark:bg-gray-800/50 rounded-lg text-sm text-gray-500 dark:text-gray-400">
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5 animate-fade-in">
      <div className={`card text-center border-l-4 ${pct >= 70 ? 'border-green-500' : pct >= 50 ? 'border-yellow-500' : 'border-red-500'}`}>
        ...
        <div className="text-3xl font-bold text-gray-900 dark:text-white mb-1">{pct}%</div>
        <div className="text-gray-500 dark:text-gray-400 text-sm">{pct >= 70 ? 'Great performance!' : pct >= 50 ? 'Good, keep practicing weak areas' : 'Needs more revision — focus on marked topics'}</div>
      </div>

      <div className="grid grid-cols-3 gap-3">
        {[
          ['Nailed It', easy,  'text-green-500 dark:text-green-400',  'bg-green-50 dark:bg-green-900/20'],
          ['Partially', med,   'text-yellow-500 dark:text-yellow-400', 'bg-yellow-50 dark:bg-yellow-900/20'],
          ['Missed',   hard,  'text-red-500 dark:text-red-400',       'bg-red-50 dark:bg-red-900/20'],
      ...
      {hard > 0 && (
        <div className="card">
          <div className="text-sm font-semibold text-red-500 dark:text-red-300 mb-2">Review these — you missed them:</div>
          <div className="space-y-2">
            {scores.filter(s => s.rating === 'hard').map((s, i) => (
              <div key={i} className="text-xs text-gray-600 dark:text-gray-400 p-2 bg-gray-100 dark:bg-gray-800 rounded">• {s.q.question}</div>
            ))}
          </div>
        </div>
      )}
