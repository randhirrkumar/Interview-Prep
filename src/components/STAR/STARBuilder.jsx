    ...
    <div className="max-w-4xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="card">
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          {['Situation', 'Task', 'Action', 'Result'].map(s => (
            <span key={s} className="tag bg-purple-900/40 text-purple-300">{s}</span>
          ))}
        </div>
      </div>
      ...
      {!isFormOpen && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-gray-700 hover:border-blue-600 rounded-xl py-4 text-gray-500 hover:text-blue-400 transition-colors"
        >
          <Plus size={18} />
      ...
      {isFormOpen && (
        <div className="card border-blue-800/50">
          <h2 className="text-sm font-semibold text-blue-300 mb-4">{editing !== null ? 'Edit Story' : 'New STAR Story'}</h2>

          {/* Question */}
          <div className="space-y-1 mb-4">
            <label className="text-xs font-semibold text-gray-400 uppercase">Interview Question</label>
            <input
              value={form.question}
              onChange={e => setForm(f => ({ ...f, question: e.target.value }))}
              placeholder="e.g. Tell me about a time you handled a conflict..."
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-blue-600"
            />
            {/* Sample questions */}
            <div className="flex flex-wrap gap-1 mt-1">
              {SAMPLE_QUESTIONS.slice(0, 3).map((q, i) => (
                <button key={i} onClick={() => setForm(f => ({ ...f, question: q }))}
                  className="text-xs text-gray-600 hover:text-blue-400 underline transition-colors">
                  {q.slice(0, 40)}...
                </button>
              ))}
            </div>
          ...
          {[
            ...
            <div key={key} className="space-y-1 mb-4">
              ...
              <textarea
                ...
                rows={3}
                className={`w-full bg-gray-800 border border-gray-700 border-l-4 ${color} rounded-lg px-3 py-2 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-r-blue-600 focus:border-t-blue-600 focus:border-b-blue-600 resize-none`}
              />
            </div>
          ))}

          <div className="flex gap-2 justify-end">
            <button onClick={cancel} className="flex items-center gap-1 text-sm text-gray-500 hover:text-white px-4 py-2 rounded-lg border border-gray-700 transition-colors">
              <X size={14} /> Cancel
            </button>
          ...
          <div key={story.id} className="card">
            <div className="flex items-start gap-3 cursor-pointer" onClick={() => setExpanded(expanded === idx ? null : idx)}>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-white line-clamp-2">{story.question}</div>
                {!expanded === idx && (
            ...
            {expanded === idx && (
              <div className="mt-4 space-y-3 border-t border-gray-800 pt-4">
                {[
                  { key: 'situation', label: 'Situation', color: 'border-l-blue-500 bg-blue-900/10' },
                  { key: 'task', label: 'Task', color: 'border-l-yellow-500 bg-yellow-900/10' },
                  { key: 'action', label: 'Action', color: 'border-l-orange-500 bg-orange-900/10' },
                  { key: 'result', label: 'Result', color: 'border-l-green-500 bg-green-900/10' },
                  ...
                  <div key={key} className={`border-l-4 ${color} rounded-r-lg p-3`}>
                    ...
                    <div className="text-sm text-gray-300 whitespace-pre-wrap">{story[key]}</div>
                  </div>
                ))}
              </div>
            )}
