  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 animate-fade-in">
      <div className="card">
        ...
        <p className="text-sm text-gray-400">Structured daily plan to crack Java Backend Developer interviews in 1 month. Consistent daily effort beats random preparation.</p>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {roadmapData.weeks.map(w => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm transition-colors ${activeWeek === w.week ? 'bg-blue-700 text-white' : 'bg-gray-900 border border-gray-700 text-gray-400 hover:text-white'}`}
          >
            Week {w.week}: {w.title}
          </button>
        ))}
      ...
      {week && (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="card border-l-4 border-blue-500">
            ...
            <div className="text-xl font-bold text-white">{week.title}</div>
            <div className="text-sm text-gray-400">{week.theme}</div>
          </div>
...
function DayCard({ day }) {
  ...
  return (
    <div className={`card transition-all ${done ? 'border-green-800/50 opacity-70' : 'border-gray-800'}`}>
      <div className="flex items-start gap-3">
        <button onClick={toggle} className="flex-shrink-0 mt-1">
          {done
            ? <CheckCircle2 size={20} className="text-green-500" />
            : <div className="w-5 h-5 border-2 border-gray-600 rounded-full hover:border-blue-500 transition-colors" />
          }
        </button>
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className="font-semibold text-white text-sm">{day.date}</span>
            <span className="flex items-center gap-1 text-xs text-gray-500"><Clock size={11} />{day.duration}</span>
            {day.mock && <span className="flex items-center gap-1 text-xs bg-purple-900/40 text-purple-300 px-2 py-0.5 rounded"><MessageSquare size={10} />Mock Interview</span>}
          </div>
