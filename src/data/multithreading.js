...
public class NotificationService {
    ...
    public CompletableFuture<Void> sendEmail(String to, String msg) {
      ...
      followUp: [
        { question: 'What is the difference between start() and run()? (start() creates new thread, run() runs in current thread)', answer: `start() creates a NEW thread and calls run() on it — actual parallelism. run() just calls the method on the CURRENT thread — no new thread created, no parallelism. This is a classic trap: calling thread.run() looks like it works but runs synchronously.` },
        { question: 'What is ExecutorService? What types of thread pools does Executors provide?', answer: `newFixedThreadPool(n) — fixed number of threads, others queue. newCachedThreadPool() — grows as needed, idle threads removed after 60s (good for short bursts). newSingleThreadExecutor() — one thread, tasks run in order. newScheduledThreadPool(n) — for scheduling tasks with delay/periodic.` },
        { question: 'What is the difference between Runnable and Callable?', answer: `Runnable.run() returns void and cannot throw checked exceptions. Callable.call() returns a value of type V and CAN throw checked exceptions. ExecutorService.submit(Callable) returns a Future<V> — call future.get() to retrieve the result (blocks until done).` },
      ],
...
AtomicReference<Status> status = new AtomicReference<>(Status.ACTIVE);`,
      followUp: [
        { question: 'What is the difference between synchronized and ReentrantLock?', answer: `synchronized is simpler — compiler handles lock/unlock. ReentrantLock gives more control: tryLock(timeout) to avoid deadlocks, lockInterruptibly() to let threads respond to interruption, fair mode (threads acquire lock in order they requested it), multiple Condition objects (for complex wait/notify). Always unlock ReentrantLock in a finally block. Use synchronized for simple cases; ReentrantLock for more complex scenarios.` },
        { question: 'What is a deadlock? Give an example.', answer: `Deadlock: Thread A holds Lock 1, waits for Lock 2. Thread B holds Lock 2, waits for Lock 1. Neither can proceed. Prevention: (1) Always acquire locks in the same order (e.g., always lock the lower ID first). (2) Use tryLock() with a timeout — if can't acquire, release held locks and retry. (3) Avoid holding multiple locks simultaneously. (4) Use higher-level concurrency utilities instead of raw locks.` },
        { question: 'What is volatile keyword?', answer: `volatile guarantees VISIBILITY — every read of a volatile variable gets the latest value written by any thread (no CPU cache stale reads). But volatile does NOT guarantee ATOMICITY — volatile int count++  is still NOT thread-safe (it's still read-modify-write). Use volatile for: boolean flags (volatile boolean running = true), singleton double-checked locking pattern.` },
      ],
