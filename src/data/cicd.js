  ...
  questions: [
    {
      ...
      difficulty: 'beginner',
      tags: ['CI/CD', 'Overview'],
      answer: `CI/CD stands for Continuous Integration and Continuous Delivery/Deployment — a practice where code changes are automatically built, tested, and deployed to production with minimal manual intervention.

Continuous Integration (CI) — every code push triggers an automated build and test run. Developers integrate their changes frequently (multiple times per day), and the pipeline catches integration bugs, failing tests, and code quality issues immediately — not days later.

Continuous Delivery (CD) — every successful CI build produces a release artifact (Docker image, JAR) that is deployable to production. The deployment to production is triggered manually.

Continuous Deployment — every successful pipeline run deploys automatically to production without manual approval.

Why it matters in microservices:

Multiple teams, multiple services — without CI/CD, coordinating deployments of 20 microservices across 5 teams manually is chaos. CI/CD makes each service independently deployable on its own cadence.

Faster feedback loops — a developer pushing code knows within 10 minutes if their change broke anything, not after weeks in a QA cycle.

Consistent deployments — every deployment goes through the same pipeline steps (build → test → static analysis → security scan → deploy). No snowflake manual deployments.

Rollback capability — every build is a versioned Docker image tagged by git commit hash. Rolling back means deploying the previous image tag.

Compliance and auditability — the pipeline records every deployment: who triggered it, which commit, when, to which environment. Full deployment audit trail without manual records.`,
      followUp: {
        question: 'What is the difference between Continuous Delivery and Continuous Deployment?',
        answer: `Continuous Delivery automates the pipeline up to the point of production deployment but requires a human to approve the final push to production. The artifact is always in a deployable state, but a manual gate controls when it goes live. This is appropriate for organizations with regulatory requirements, customer commitments around change windows, or features that need coordination with marketing launches. Continuous Deployment takes the human gate out entirely — every commit that passes all pipeline stages goes to production automatically. This requires very high confidence in automated tests, feature flags to control feature visibility independent of deployment, and excellent monitoring to detect production issues quickly. Most enterprises practice Continuous Delivery with automatic deployment to dev/staging and manual approval for production.`
      }
    },
    {
      id: 'cicd_q2',
...
- Developer pushes to a feature branch → CI pipeline starts
...
- Slack notification: "Build ready for production — approve?"
...
The Docker image tag is always the git commit SHA — this makes deployments fully traceable to source code. Never use latest in production.`,
      followUp: {
        question: 'How do you handle database migrations in a CI/CD pipeline?',
        answer: `Database migrations (Flyway or Liquibase) run as part of the Spring Boot application startup — when the application starts in staging or production, it applies pending migrations. This means migrations must always be backward compatible with the previous application version because the old version may still be running during a rolling update when the new version starts and runs migrations. Practices: 1. Never drop a column in the same release that stops using it — first release removes the code reference, second release adds the Flyway script to drop the column. 2. New columns added with DEFAULT or nullable — so the old application version can still write rows without the new column. 3. Some teams run migrations as a Kubernetes Job before the rolling Deployment update (initContainer or pre-deploy Job). This ensures migrations complete before new pods start, but requires the migration to not break the old pods still running.`
      }
    },
    {
      id: 'cicd_q3',
...
This is different from traditional CI/CD where the pipeline imperatively runs kubectl apply or helm upgrade — in GitOps, the cluster pulls from Git, not the pipeline pushing to the cluster.

Benefits:
- Audit trail — every cluster change is a Git commit with author, timestamp, and diff
- Rollback = git revert — revert the commit, ArgoCD applies the previous state
- Disaster recovery — rebuild any environment from the Git repository
- No kubectl access required for developers — changes happen through PRs
...
waitForQualityGate in Jenkins (abortPipeline: true) polls the SonarQube server after analysis completes and fails the pipeline if the quality gate fails — this is necessary because sonar:sonar returns success even if the gate fails; the gate result is computed asynchronously.`,
      followUp: {
        question: 'What are the key metrics SonarQube tracks and what do they mean?',
        answer: `Bugs — code patterns that are likely to produce incorrect behavior. Counted, weighted by severity (Blocker, Critical, Major, Minor, Info). Vulnerabilities — security weaknesses (SQL injection, hardcoded credentials, XSS). These are the highest priority to fix. Code Smells — maintainability issues (overly complex methods, duplicated code, long parameter lists) that make code hard to maintain but don't directly cause bugs. Technical Debt — an estimate (in hours/days) of how long it would take to fix all code smells. Coverage — what percentage of lines/branches are executed by tests. SonarQube uses JaCoCo report for Java. Duplications — copy-pasted code that should be extracted into shared methods. Security Hotspots — code that requires a security review but might not be a vulnerability — e.g., use of a cryptographic function. The developer must mark it as Reviewed (safe) or as a Vulnerability to fix.`
      }
    },
    {
      id: 'cicd_q7',
...
Rolling Deployment — gradually replaces old pods with new ones. Traffic shifts to new pods as they become ready. Old and new versions coexist temporarily.
...
Blue-Green Deployment — two full environments (Blue = old, Green = new). Green is deployed and tested while Blue handles all traffic. Traffic switches atomically (DNS change or load balancer rule change) from Blue to Green.
- Risk: low — you test Green fully before any user sees it; rollback is instant (switch back to Blue)
...
Canary Deployment — new version is deployed to a small percentage of pods (e.g., 5%). A subset of real users hit the new version. Monitor metrics (error rate, latency) on the canary. Gradually increase traffic percentage if metrics are healthy, until 100%.
